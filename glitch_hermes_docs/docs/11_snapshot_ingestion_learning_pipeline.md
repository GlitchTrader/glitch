...
One Glitch publisher owns each minute and retries it until market and portfolio snapshots with the same `snapshot_id` form one immutable frame. Every new frame publishes a packet from the five latest observed frames. Continuity, observed span, and missing-minute IDs are explicit, so one absent minute is uncertainty evidence rather than a four-minute blackout. The lightweight worker wakes on Hermes's native one-minute tick and invokes Luna after five elapsed minutes while flat, on every new packet while positioned, once for an explicit directive, or on the next packet after any model, validation, delivery, firewall, or executor failure.

Hermes native cron owns the wake-up under a supervised gateway. Its worker performs a zero-model check for a new packet, creates an isolated `trading`-tagged session for every eligible model call, resends bounded Glitch decision/execution/outcome tails, supplies a literal valid-output template, and submits strict intents to Glitch's authenticated receiver. Transport uncertainty resubmits the identical durable outbox; terminal failure requests new cognition on the next packet. It does not classify opportunities or impose trading archetypes before inference. Contract/scope validation cannot replace Hermes's probabilistic decision; Glitch's firewall remains the execution authority.
