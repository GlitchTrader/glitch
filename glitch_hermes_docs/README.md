...
- **2026-07-13, amended 2026-07-20 — Direct control and sessions:** one `glitch` profile owns the named human `chat` session. The core worker uses a fresh bounded decision context for every eligible packet; deterministic slash commands call Glitch's authenticated control endpoint and are reflected in the Glitch UI. Native Kanban is deferred to learning/review and is never an execution rail.
...
- **2026-07-09, reconciled 2026-07-20 — Hermes operator contract:** `docs/10_hermes_operator_contract.md` defines the active local five-minute Sim/paper validation loop, required input bundle, strict JSON output, learning boundary, and target centralized transport. The worker runs under a supervised gateway with one fresh bounded decision context per eligible packet; Codex is not a scheduler or relay.
