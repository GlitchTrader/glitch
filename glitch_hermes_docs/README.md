...
- **2026-07-09, reconciled 2026-07-19 — Hermes operator contract:** `docs/10_hermes_operator_contract.md` defines the active local five-minute Sim/paper validation loop, required input bundle, strict JSON output, learning boundary, and target centralized transport. The worker runs under a supervised gateway and one named `trading` session; Codex is not a scheduler or relay.
