...
Текущие каналы Glitch предлагают два полных взаимоисключающих пакета: Standard v0.0.2.5 и Experimental AI v0.0.2.6. AI v0.0.2.6 — это текущая временная продвиженная версия основного продукта в `main`.
...
Коэффициент ведомого счёта меняет **количество** в его ордере. Он не создаёт дополнительные независимые ордера. При `2x` ведомый счёт получает удвоенное количество в одном собственном потоке ордеров, подчиняясь только нативному решению NinjaTrader и видимым правилам соответствия, которые включил пользователь.
...
- установлен `Glitch_AI_v0.0.2.6.zip`;
...
- `glitch-learning-supervisor` — проверяется каждые 30 минут.
...
- Учебный супервайзер просыпается каждые 30 минут и по расписанию запускает разбор сделок, часовую проверку, планирование на 300 минут и ежедневный Journal.
...
Hermes отвечает за мышление, стратегию и предложения количества на мастере. Glitch проверяет фактическую привязку счетов и групп, нативное состояние, геометрию, исполнение, brackets, репликацию и receipts. Ёмкость, риск, сессия и compliance остаются данными для Hermes и UI; они не могут молча отклонять намерение AI без конкретного действия, включённого пользователем в Settings. Намерение человека выше Hermes, Hermes выше детерминированного вывода, а NinjaTrader остаётся авторитетом для нативных результатов. Переключатель paper/live не меняет полномочия счетов.
