...
跟随比例改变的是跟随订单的**数量**，不会创建额外的独立订单。`2x` 跟随账户会在一个本地订单流程中获得主账户两倍的数量，仅受 NinjaTrader 原生接受结果和用户已启用的可见合规控制约束。
...
- `glitch-learning-supervisor` — 每 30 分钟检查。
...
- 学习监督器每 30 分钟唤醒，并在相应周期到期时执行交易复盘、小时监督、300 分钟规划和每日 Journal。
...
Hermes 负责认知、策略和主账户数量建议。Glitch 验证账户和分组绑定、原生状态、几何结构、执行、brackets、复制和 receipts 等事实。容量、风险、交易时段和合规信息会提供给 Hermes 与 UI；除非用户在 Settings 中明确启用对应操作，否则它们不能静默否决 AI 意图。人工意图优先于 Hermes，Hermes 优先于确定性推断；NinjaTrader 仍是原生结果的权威来源。paper/live 开关不会改变账户权限。
