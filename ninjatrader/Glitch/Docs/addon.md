...
- **GlitchRuntimePolicyStore** — Runtime policy settings and license cache paths (`RuntimePolicy.tsv`, `LicenseCache.tsv`); load/save settings (compliance toggles, license key, API base URL, installation ID); license cache state (signed token, normalized plan, billing variant, source product/plan IDs, feature flags, grace window).
...
- **GlitchLicenseService** — License validation and heartbeat against the Glitch API; ES256 token verification; normalized policy plus token claims (`BillingVariant`, `SourceProductId`, `SourcePlanCode`) for diagnostics/UI; canonical API base URL and allowed hosts.
