...
{
    // Apex directionality is a portfolio rule, not a replication-group rule.
    // Every Glitch-owned entry path calls this guard immediately before submit.
    internal static class GlitchApexDirectionGuard
    {
        public static bool TryApproveEntry(
            ...
            int portfolioSnapshotMaxAgeSeconds,
            out string failure)
        {
            failure = null;
            ...
            bool classificationsFresh = GlitchAiPortfolioSnapshotReader.TryGetFreshAccountClassifications(
                DateTime.UtcNow,
                Math.Max(1, portfolioSnapshotMaxAgeSeconds),
                out IReadOnlyList<GlitchPortfolioAccountClassification> classifications,
                out string classificationFailure);

            GlitchPortfolioAccountClassification proposedClassification = classificationsFresh
                ? classifications.FirstOrDefault(item => item != null
                    && string.Equals(item.AccountName, proposedAccount.Name, StringComparison.OrdinalIgnoreCase))
                : null;
            string proposedFirm = proposedClassification?.PropFirmId;
            string proposedStatus = proposedClassification?.AccountStatus;
            if (string.IsNullOrWhiteSpace(proposedFirm))
                proposedFirm = GlitchComplianceEngine.InferPropFirmId(proposedAccount, out _);
            if (string.IsNullOrWhiteSpace(proposedStatus))
                proposedStatus = GlitchComplianceEngine.InferAccountStatus(proposedAccount, proposedFirm, out _);

            // Sim is an isolated test environment even when its data connection is Apex.
            if (string.Equals(proposedStatus, "Sim", StringComparison.OrdinalIgnoreCase)
                || !IsApexFirm(proposedFirm))
                return true;

            if (!classificationsFresh)
            {
                failure = "apex_portfolio_classification_unavailable|reason=" + CleanToken(classificationFailure);
                return false;
            }
            if (proposedClassification == null)
            {
                failure = "apex_proposed_account_not_in_portfolio_snapshot|account="
                    + CleanToken(proposedAccount.Name);
                return false;
            }
            ...
            {
                lock (Account.All)
                    liveAccounts = Account.All.Where(account => account != null).ToArray();
            }
            catch
            {
            ...
            foreach (GlitchPortfolioAccountClassification classification in classifications)
            {
                if (classification == null
                    || string.Equals(classification.AccountStatus, "Sim", StringComparison.OrdinalIgnoreCase)
                    || !IsApexFirm(classification.PropFirmId))
                    continue;

                Account account = liveAccounts.FirstOrDefault(item => string.Equals(
                    item.Name,
                    classification.AccountName,
                    StringComparison.OrdinalIgnoreCase));
                if (account == null)
                {
                    failure = "apex_account_state_unavailable|account=" + CleanToken(classification.AccountName);
                    return false;
                }
                ...
                {
                    if (position?.Instrument == null || position.MarketPosition == MarketPosition.Flat || position.Quantity <= 0)
                        continue;
                ...
                {
                    if (order?.Instrument == null || !GlitchReplicationEngine.IsWorkingOrderState(order.OrderState))
                        continue;
