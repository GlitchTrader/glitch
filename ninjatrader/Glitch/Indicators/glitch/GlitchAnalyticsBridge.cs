    ...
    public class GlitchAnalyticsBridge : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.Terminated)
            {
                ...
                BridgeBusCompat.UnregisterBridgeBootstrapPublisher(_instrumentRoot, RequestBootstrapFromExternal);
                BridgeBusCompat.UnregisterBridge(_instrumentRoot);
            }
        }
        ...
        private static class BridgeBusCompat
        {
            ...
            private static bool EnsureResolved()
            {
                lock (SyncRoot)
                {
                    ...
                    Type busType = ResolveType("Glitch.UI.GlitchAnalyticsFeedBus");
                    Type readingType = ResolveType("Glitch.UI.GlitchIndicatorReading");
		...
		public GlitchAnalyticsBridge GlitchAnalyticsBridge(ISeries<double> input, double neutralBand, bool enableBarColoring, bool publishToGlitchUi, int publishIntervalMs, bool intraBarColoring, double predictiveBoost, double flipHysteresis, bool performanceMode, bool enableOrderFlowLayer, double orderFlowBlend)
		{
			if (cacheGlitchAnalyticsBridge != null)
				for (int idx = 0; idx < cacheGlitchAnalyticsBridge.Length; idx++)
					if (cacheGlitchAnalyticsBridge[idx] != null && cacheGlitchAnalyticsBridge[idx].NeutralBand == neutralBand && cacheGlitchAnalyticsBridge[idx].EnableBarColoring == enableBarColoring && cacheGlitchAnalyticsBridge[idx].PublishToGlitchUi == publishToGlitchUi && cacheGlitchAnalyticsBridge[idx].PublishIntervalMs == publishIntervalMs && cacheGlitchAnalyticsBridge[idx].IntraBarColoring == intraBarColoring && cacheGlitchAnalyticsBridge[idx].PredictiveBoost == predictiveBoost && cacheGlitchAnalyticsBridge[idx].FlipHysteresis == flipHysteresis && cacheGlitchAnalyticsBridge[idx].PerformanceMode == performanceMode && cacheGlitchAnalyticsBridge[idx].EnableOrderFlowLayer == enableOrderFlowLayer && cacheGlitchAnalyticsBridge[idx].OrderFlowBlend == orderFlowBlend && cacheGlitchAnalyticsBridge[idx].EqualsInput(input))
