...
2. Use `/long` or `/short` only for an operator-directed paper experiment. The command writes a bounded directive; the next stateless worker cycle still calculates structure and emits the decision.
