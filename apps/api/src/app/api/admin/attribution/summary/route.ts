...
import { EntitlementStoreConfigError, listEntitlementAttributionSummary } from "@/lib/entitlements-store";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const summary = await listEntitlementAttributionSummary(limit);
    return jsonResponse({
      ok: true,
      requestId,
    ...
    return errorResponse(
      requestId,
      500,
      "attribution_summary_error",
      "Failed to build attribution summary.",
