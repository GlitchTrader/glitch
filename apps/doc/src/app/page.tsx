...
export default function DocsHomePage() {
  ...
  const jsonLd = [
    {
      ...
      "@type": "CollectionPage",
      name: "Glitch Docs - NinjaTrader AddOn Documentation",
  ...
  const installationGuideDoc =
    docs.find((doc) => doc.slug === "installation-guide-troubleshooting") ?? null;
  const gridDocs = docs.filter((doc) => doc.slug !== "installation-guide-troubleshooting");
  ...
  return (
    <DocsShell activeSlug={null}>
      ...
      <div className="space-y-8">
        ...
        {installationGuideDoc ? (
          <section>
            <Link
              href={installationGuideDoc.href}
              className="group block rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6 transition hover:border-glitch-teal/40 hover:bg-white/[0.05] sm:p-7"
            >
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-glitch-teal">
                {installationGuideDoc.section}
              </p>
              <h2 className="mt-3 text-2xl font-semibold text-white transition group-hover:text-glitch-teal sm:text-[2rem]">
                {installationGuideDoc.navTitle}
              </h2>
              <p className="mt-3 max-w-4xl text-base leading-7 text-zinc-300">{installationGuideDoc.summary}</p>
