...
function websitePath(pathname: string): string {
  return `${trimTrailingSlash(websiteUrl)}${pathname}`;
}
    ...
    <header className="sticky top-0 z-50 border-b border-zinc-200 bg-white/95 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-6 text-sm md:flex">
          <a href={websitePath("/")} className={navLinkClass}>
            Home
          </a>
          <a href={websitePath("/offer")} className={navLinkClass}>
            Offer
          </a>
          <a href={websitePath("/pricing")} className={navLinkClass}>
            Pricing
          </a>
          <a href={websitePath("/affiliate")} className={navLinkClass}>
            Affiliate
          </a>
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-zinc-200 pt-6 text-sm text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
          ...
          <div className="flex flex-wrap gap-6">
            ...
            <a href={websitePath("/risk-disclosure")} className="hover:text-zinc-700 dark:hover:text-zinc-300">
              Risk Disclosure
            </a>
            <a href={websitePath("/terms")} className="hover:text-zinc-700 dark:hover:text-zinc-300">
              Terms
            </a>
            <a href={websitePath("/privacy")} className="hover:text-zinc-700 dark:hover:text-zinc-300">
              Privacy
            </a>
