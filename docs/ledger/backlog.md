...
**Active work:** `docs/ai-program/operating-system-rail.md` on clean candidate
**`cleanup/ai-core`**; shared non-AI promotion work uses **`cleanup/main-core`**.
...
|----|--------------------|------------------------------|-----------------------------|
| GL-042 | **R12a — master-only AI executor; shared replication integration consolidated.** Depends on R08–R10 and the producer-neutral CopyEngine. | The configured AI executor resolves one allowlisted group master, validates current master risk and execution admission, creates the master's OCO bracket, and manages only master orders. `GlitchCopyEngine` separately owns followers, ratios, and follower-local admission. A follower refusal never cancels a valid master decision. AI never submits, moves, cancels, or flattens follower orders. | Historical direct-rail Sim acceptance exists. The clean consolidation still requires F5 plus bounded master/follower lifecycle evidence before promotion. |
| GL-043 | **R11a — strict Hermes ingress.** Depends on GL-042 policy surface and R07 telemetry. | `tools/hermes/` direct adapter and tests; `glitch_hermes_docs/schemas/intent.v2.schema.json`; `GlitchAiIntentValidator.cs`. Read the latest rolling packet and policy; create an isolated session tagged `trading` under a hard timeout and four-turn ceiling; resend bounded recent decisions/executions/outcomes; supply a scoped valid JSON template; extract and strictly validate one batch; persist one outbox batch per packet before posting. Model/contract errors produce no order and make the next newer pre-boundary packet eligible. | Entries are `MARKET`, bracket-mandatory, and dynamically sized from valid master quantities. `MOVE_TP` moves all remaining targets and may tighten stops. Completed receipts stop replay; delivery retries reuse identical intent IDs and no packet gets a second model call. |
| GL-044 | **H-2 profile — one Glitch Hermes operator (source complete; fresh runtime proof pending).** Depends on GL-043 contract, not on live execution. | Canonical overlays live under `hermes-profile/skills`; SOUL and skills preserve Hermes native memory while Glitch/NinjaTrader remains fact authority. The host `glitch` profile is isolated from Workframe dogfood, uses a local terminal backend, creates isolated `trading`-tagged scheduled sessions, pins `gpt-5.6-luna`/OpenAI Codex with medium reasoning, clears silent fallbacks, and receives bounded current Glitch packet/ledger context. | Installer/source tests prove exact overlay reconciliation, model/provider/effort pinning, no fallback chain, and preserved native memory/session infrastructure. Remaining after deployment: one read-only memory canary and proof that profile restart retains durable lessons without replaying stale decisions. |
...
| GL-047 | **S0 — gateway and cycle continuity; scheduler isolation runtime-proven, broader recovery proof open.** Blocks all expansion. | Hermes-native supervised hidden service, exactly one `glitch` profile whose scheduled calls use isolated sessions tagged `trading`, explicit Luna/OpenAI-Codex/medium routing with no silent fallback, four-turn cycle bound, next-newer-packet retry after any failed decision, zero Codex/operator polling in runtime, and crash-safe packet outbox/receipt continuity. Slow learning is launched into a separately locked process so it cannot occupy native cron's serialized minute-operator lane. | Source installer/idempotency tests are green; installed scripts match source and exactly the two intended no-agent jobs are enabled. The first scheduled launcher returned in 198 ms; while its worker remained active, direct cron continued and completed a Luna decision. Remaining: prove terminal/Codex exit does not stop scheduled work, gateway status agrees across service and Hermes surfaces, restart-on-failure works, a killed worker reuses the same outbox/intent IDs with zero new model calls, and missed windows are journaled. |
| GL-048 | **S0 — authoritative master outcome learning; recursive runtime active, later cognition evaluation open.** Depends on completed Glitch journal + TradeLedger rows. | Correlate each master lifecycle with its AI receipt, authoritative `TradeLedger.tsv` round trip, execution/management events, minute excursion path, and a later master-terminal portfolio snapshot. Follower round trips and protection remain replication diagnostics; a follower miss cannot suppress master attribution. `pnl_points` is already quantity-weighted, so currency PnL multiplies by point value once and subtracts commission. Close kind comes from execution metadata. Every master trade becomes an episodic debrief; repeated attributable episodes may update native memory and one versioned, reversible cognitive overlay. | Source tests prove master learning remains eligible through missing/divergent follower evidence, terminal MFE/MAE bounds, append-only 15/60/300-minute/daily stores, later-evidence promotion/rollback, and isolated `trading` sessions. The first corrected scheduled worker debriefed the newest eight of 37 eligible outcomes, including the latest two wins, increased durable episodes from 24 to 32, and published a new hourly review/current guidance. Debrief evidence exposes one unambiguous master learning authority and separates follower diagnostics. Remaining: later repeated evidence must change memory/cognition and a subsequent untouched window must evaluate that change. |
...
| GL-050 | **S1 — observable Feed; local UI slice implemented, localized, and compiled.** Depends on GL-047–049b green for acceptance and on GL-052 for central-client transport. | The local read-only `Glitch AI` Feed separates the current five-snapshot window from the latest completed decision, shows snapshot/decision ages, renders five stages (snapshots, sealed packet, AI decision, execution check, outcome), and retains 20 expandable decisions with matching execution and packet evidence. AI Trading Scope is a collapsed configuration section over existing group masters. No Chat or free-form customer control surface. All authored UI copy uses the six-locale catalog; model-authored reasoning remains verbatim. Adapt this same view to central recommendation receipts later; do not create a second Feed implementation. | Source/UI and localization contracts pass; the complete tab compiled in the deployed AddOn. Remaining: prove durable rebuild/freshness/error rendering across restart and preserve zero extra model calls; later bind the same view to central API receipts. |
...
| GL-053 | **S3 — quantity, legs, and group control. Local slice implemented, deployed, compiled, and Sim-proven on `ai-rail`; central recommendation portion still depends on GL-052.** | Hermes trades only the configured master. Glitch publishes valid master quantities from the master's account-wide open contracts and prop-rule ceiling only. Followers and ratios are user configuration; CopyEngine applies each route independently, with visible follower-local rejection when necessary. Up to three independent native OCO legs support scale-out, while later same-direction intents remain independently protected averaging tranches. | Current direct-rail tests pass. Master admission is independent of replication state, ratios, route completeness, and follower capacity; follower submission still uses NinjaTrader rounding and follower-local caps. Runtime proof covered three protected master legs and matching followers, plus 1:2:3 replication where one master contract produced 1/2/3 protected exposure and all accounts returned flat/order-free. Remaining work belongs to central recommendation transport and bounded performance calibration, not another local quantity gate. |
...
| GL-065 | **S0 — reload safety and follower action ownership. Protected reload runtime-proven; terminal follow-through open.** | AddOn startup/recompile is observe-only: restore UI/configuration and event subscriptions without submitting, cancelling, replacing, or catching up any order. Existing native brackets remain NinjaTrader-owned. Only exact `GLT-COPY-*` and `GLT-CATCHUP-*` grammars prove replication ownership; a broad `GLT-*` prefix is insufficient. Manual NT and Glitch controls always remain effective. A manual follower action is preserved; Glitch may alert and cancel only orphan Glitch protection after the account is actually flat, but it does not quarantine the account, silently re-enter it, or override the human. Replicate ON, follower enable, or ratio change is an explicit resync request. Flatten All remains available regardless of divergence. Master entries are checked against master and firm truth; each follower independently checks its user ratio, alignment, native state, and cap, surfaces any refusal, and never vetoes the master. | F5 at 04:32:50 UTC while a protected group was open preserved positions `1/1/2`, working-order counts `2/2/4`, and byte-identical native order IDs/prices/details in the adjacent pre/post-compile packets. Remaining: prove exactly one surviving AddOn instance observes the book and a later native TP/SL closes correctly after reload. Manual account controls and Flatten All remain effective. Replication OFF preserves existing protection. A Glitch-generated opposite-direction proposal is rejected before submission with no account mutation. Provider authentication failure is surfaced and cannot be misreported as a successful close/reconcile. |
...
### Mainline consolidation handoff — shared Glitch core

**Recorded:** 2026-07-15

**Target:** `main` / non-AI Glitch

**Historical comparison baseline:** `main`/`origin/main` at `d216015` and
`glitch/ai-rail` at `d7975fb`. Implementation has since moved to the clean
`cleanup/main-core` and `cleanup/ai-core` candidates. Compare exact candidate
commits again before promotion; do not use the historical dirty rail as source.
