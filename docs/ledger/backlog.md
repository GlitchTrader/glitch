...
| GL-047 | **S0 — gateway and cycle continuity; implementation present, runtime proof open.** Blocks all expansion. | Hermes-native supervised hidden service, exactly one `glitch` profile whose scheduled calls resume `trading`, explicit Luna/OpenAI-Codex/medium routing with no silent fallback, four-turn cycle bound, next-newer-packet retry after any failed decision, zero Codex/operator polling in runtime, and crash-safe packet outbox/receipt continuity. | Source installer/idempotency tests are green. Remaining: prove terminal/Codex exit does not stop scheduled work, gateway status agrees across service and Hermes surfaces, restart-on-failure works, a killed worker reuses the same outbox/intent IDs with zero new model calls, and missed windows are journaled. |
| GL-048 | **S0 — authoritative master outcome learning; recursive source path complete, installed-runtime evidence open.** Depends on completed Glitch journal + TradeLedger rows. | Correlate each master lifecycle with its AI receipt, authoritative `TradeLedger.tsv` round trip, execution/management events, minute excursion path, and a later master-terminal portfolio snapshot. Follower round trips and protection remain replication diagnostics; a follower miss cannot suppress master attribution. `pnl_points` is already quantity-weighted, so currency PnL multiplies by point value once and subtracts commission. Close kind comes from execution metadata. Every master trade becomes an episodic debrief; repeated attributable episodes may update native memory and one versioned, reversible cognitive overlay. | Source tests prove master learning remains eligible through missing/divergent follower evidence, terminal MFE/MAE bounds, append-only 15/60/300-minute/daily stores, later-evidence promotion/rollback, and isolated `trading` sessions. Remaining: installed worker evidence that a new completed trade produces an episode, later repeated evidence changes memory/cognition, and a subsequent untouched window evaluates that change. |
