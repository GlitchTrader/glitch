...
| GL-065 | **S0 — reload safety and follower action ownership.** Blocks declaring the local harness reliable. | AddOn startup/recompile is observe-only: restore UI/configuration and event subscriptions without submitting, cancelling, or replacing any order. Existing native brackets remain NinjaTrader-owned. Only exact Glitch signal grammars prove machine ownership; a broad `GLT-*` prefix is insufficient. Glitch-owned follower mishaps may self-heal. Unknown/manual follower executions quarantine only that follower/instrument: a human flat or position is respected, orphan Glitch protection is cancelled when flat, and divergence is alerted but never silently traded over. Every Glitch-owned entry path is pre-checked against portfolio-wide Apex correlated-direction exposure; an external/manual native fill that bypasses Glitch is preserved but refused replication and raised as critical. Explicit Replicate Off/On, follower re-enable, or ratio change is the resync boundary. | With AI and replication ON, F5 while a protected Sim group is open leaves every position, working-order ID/count/price, and PnL unchanged; exactly one surviving AddOn instance observes the book. A later native TP/SL still closes correctly after reload. Manual follower close leaves that follower flat and quarantined; later master activity does not silently re-enter it. Manual same-direction or opposite-direction change is not overridden. A Glitch-owned cross-direction proposal is rejected before submission with one compliance alert and no account mutation. Provider authentication failure is surfaced and cannot be misreported as a successful close/reconcile. |

**Mandatory order:** close runtime evidence for GL-047–049b + GL-055 + GL-065 -> prove GL-053 and GL-063 on Sim -> collect GL-064 stable one-instrument evidence -> finish GL-050 acceptance -> GL-051–052 centralization -> GL-054 instruments. GL-053's local two-leg slice already exists. No parallel implementation queue and no automatic Codex build loop.
