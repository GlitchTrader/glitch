...
$sourcePaths = @(
    ...
    (Join-Path $repoRoot 'ninjatrader\Glitch\AddOns\GlitchAddOn\Infrastructure\GlitchOperationJournal.cs'),
    (Join-Path $repoRoot 'tools\tests\GlitchJournalHarness.cs')
)
...
$compilation = [Microsoft.CodeAnalysis.CSharp.CSharpCompilation]::Create(
    'GlitchJournalHarness', $syntaxTrees, $references,
    [Microsoft.CodeAnalysis.CSharp.CSharpCompilationOptions]::new(
        [Microsoft.CodeAnalysis.OutputKind]::ConsoleApplication))
$outputPath = Join-Path ([System.IO.Path]::GetTempPath()) 'GlitchJournalHarness.exe'
...
& $outputPath
if ($LASTEXITCODE -ne 0) { throw "journal harness exited with code $LASTEXITCODE" }
