...
$sourcePaths = @(
    (Join-Path $repoRoot 'ninjatrader\Glitch\AddOns\GlitchAddOn\Services\Persistence\GlitchConfigurationStore.cs'),
    (Join-Path $repoRoot 'ninjatrader\Glitch\AddOns\GlitchAddOn\Services\Persistence\GlitchStateStore.cs'),
    (Join-Path $repoRoot 'ninjatrader\Glitch\AddOns\GlitchAddOn\Services\Persistence\GlitchEvalTargetLockStore.cs'),
    (Join-Path $repoRoot 'tools\tests\GlitchEvalTargetLockHarness.cs')
)
...
$referencePaths = @(
    ...
    [System.IO.InvalidDataException].Assembly.Location,
...
try {
    & $outputPath
    if ($LASTEXITCODE -ne 0) { throw "eval target lock harness exited with code $LASTEXITCODE" }
}
finally {
    Remove-Item -LiteralPath $outputPath -Force -ErrorAction SilentlyContinue
}
