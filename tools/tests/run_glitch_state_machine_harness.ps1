...
$sourcePaths = @(
    ...
    (Join-Path $repoRoot 'tools\tests\GlitchStateMachineHarness.cs')
)
...
$compilation = [Microsoft.CodeAnalysis.CSharp.CSharpCompilation]::Create(
    'GlitchStateMachineHarness', $syntaxTrees, $references,
    [Microsoft.CodeAnalysis.CSharp.CSharpCompilationOptions]::new(
        [Microsoft.CodeAnalysis.OutputKind]::ConsoleApplication))
$outputPath = Join-Path ([System.IO.Path]::GetTempPath()) 'GlitchStateMachineHarness.exe'
...
& $outputPath
if ($LASTEXITCODE -ne 0) { throw "state machine harness exited with code $LASTEXITCODE" }
