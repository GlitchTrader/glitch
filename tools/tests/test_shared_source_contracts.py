...
class SharedSourceArchitectureContractTests(unittest.TestCase):
    ...
    def test_native_hermes_entry_evidence_reaches_outcome_reconciliation(self):
        ...
        self.assertIn('HermesLifecycleKind = command.PropagatesAsMasterExecution ? "stop_exit"', gateway)
        self.assertIn('HermesLifecycleKind = command.PropagatesAsMasterExecution ? "target_exit"', gateway)
        self.assertIn('Append("|realized_pnl_usd=")', gateway)
        self.assertIn("metadata.HermesEntrySignedQuantity > 0 ? 1.0 : -1.0", gateway)
