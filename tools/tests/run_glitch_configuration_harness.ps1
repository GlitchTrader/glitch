...
$sourcePaths = @(
    ...
    (Join-Path $repoRoot 'tools\tests\GlitchConfigurationHarness.cs')
)
...
try {
    & $outputPath
    if ($LASTEXITCODE -ne 0) { throw "configuration harness exited with code $LASTEXITCODE" }
}
finally {
    Remove-Item -LiteralPath $outputPath -Force -ErrorAction SilentlyContinue
}
