...
class AiSourceArchitectureContractTests(unittest.TestCase):
    ...
    def test_control_receipts_are_durable_bounded_and_native_complete(self):
        ...
        self.assertIn("glitch.control.receipt.v3", control)
        self.assertIn("body_hash", control)
        self.assertIn("ComputeBodyHash", control)
        self.assertIn("command_conflict", control)
        ...
        self.assertIn("GetFlattenEvidence", telemetry)
        self.assertIn("BuildFlattenEvidenceFromNativeState", telemetry)
    ...
    def test_control_replication_is_configuration_only_and_content_bound(self):
        control = source(CONTROL_SERVER)
        self.assertIn('action == "REPLICATE_ON"', control)
        self.assertIn('action == "REPLICATE_OFF"', control)
        self.assertIn("SetReplication", control)
        self.assertIn("GetReplicationEffective", control)
        self.assertNotIn("SyncGroupFollowers", control)
        self.assertNotIn("Submit(", control)
        self.assertNotIn("Account.Flatten", control)
        self.assertIn("canonical = schemaVersion +", control)
        self.assertIn("SHA256.Create", control)
    ...
    def test_ai_move_target_changes_master_and_replication_mirrors_target_and_optional_stop(self):
        ...
        self.assertIn('pending.IsStop ? "-S-" : "-T-"', mirror)
        ...
        self.assertIn("pending.Account.Change(changes.ToArray())", mirror)
