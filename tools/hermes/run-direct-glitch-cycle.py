...
# Only legacy unit-test packets lack the NinjaTrader descriptive economics.
# Live packets must carry these values from MasterInstrument; this fallback is
# deliberately labelled so it cannot be mistaken for native economics.
...
def resolve_instrument_economics(instrument: dict[str, Any]) -> dict[str, Any]:
    """Resolve economics from the NT descriptive layer, never from profile constants."""
...
def validate_forecast(forecast: Any, index: int) -> None:
    """Validate optional Hermes forecast metadata without using it as a gate."""
    if forecast is None:
        return
...
def _attach_observation_layers(instrument: dict[str, Any]) -> None:
    ...
    if not isinstance(projections, dict):
        projections = {}
    projections.setdefault("source", "glitch_analytics_bridge_legacy")
    projections.setdefault("strategy_semantics", "none")
...
def submit_batch(batch: dict[str, Any], glitch_data: Path, exchange: Path) -> dict[str, Any]:
    ...
    for intent in batch["decisions"]:
        ...
        # Forecasts are Hermes-only attribution metadata. Glitch receives the
        # bounded intent contract; reconciliation joins this field later.
...
def build_prompt(
    ...
    prompt = (
        ...
        "Optionally include forecast exactly as {event:\"STOP_BEFORE_PRIMARY_TARGET\",probability:number,method:string,confidence:number}; it is recorded for "
        "calibration only and never acts as a deterministic entry, management, or execution gate. "
