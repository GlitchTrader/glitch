...
`launch-direct-glitch-cycle.py` is the fast minute cron boundary; it starts the separately PID-locked `run-direct-glitch-cycle.py` in a hidden detached process and returns immediately. That prevents a model call crossing a minute boundary from suppressing the next positioned packet. The worker spends zero model calls until a new complete packet exists. It invokes Luna in an isolated `trading` session with current market/portfolio truth, master-only valid quantities, stable Glitch leg IDs and native working-order geometry, recent outcomes, the active plan/guidance/cognitive overlay, native memory, and a literal valid JSON template. Flat books use elapsed five-minute cadence; positioned books use every new packet. Any model, validation, delivery, firewall, or executor failure makes the next packet immediately eligible. A bounded rollover check prevents the launcher tick from selecting the prior packet when publication is milliseconds behind the wake-up; the chosen immutable packet remains the decision identity and Glitch revalidates current execution truth. Transport-uncertain delivery reuses the same durable outbox and intent UUIDs. There is no in-process polling retry.
